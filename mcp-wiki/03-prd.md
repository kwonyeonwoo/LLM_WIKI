...
Status: active (MVP M1~M5 구현·검증 완료)
...
| `list_pages` | read | `domain?: str`, `subdir?: str` | 페이지 메타 목록 | FR-3 |
...
검증: 2026-06-11 end-to-end 테스트 (commit e961717). `pip install -e .`로 mcp SDK 설치 후 실행.

- [x] 6개 tool이 MCP 클라이언트에서 호출되고 스키마가 노출된다. — stdio initialize handshake OK (`cv-physical-ai-wiki`, mcp 1.27.2), 6 tool 등록 확인.
- [x] `create_page` 호출 시 파일 생성 + `index.md` 엔트리 + `log.md` append가 한 번에 일어난다. — write tool 격리 테스트 통과 후 git restore 복원.
- [x] `search_wiki`가 키워드로 관련 CV/Physical AI 페이지를 반환한다.
- [x] `link_check`가 끊긴 링크·색인 누락을 탐지한다. — 끊긴 링크 0 확인.
- [x] source of truth가 markdown 파일로 유지된다 (DB 의존 없음).
- [~] 시드 페이지([01](01-knowledge-domain.md) §5) 7개 존재. 단 최초 작성은 직접 Write로 수행, `create_page` tool 경로는 throwaway 페이지로 별도 검증. → 시드를 tool 경로로 재생성하는 검증은 미수행(Open Question).

## 9. 마일스톤

| 단계 | 산출물 | 상태 |
|---|---|---|
| M1 | MCP 서버 스캐폴드 + read tool 3종 (search/read/list) | ✅ 완료 |
| M2 | write tool 2종 (create/update) + index 자동 갱신 | ✅ 완료 |
| M3 | `link_check` + log append + 안전 불변조건 | ✅ 완료 |
| M4 | 시드 페이지 ingest, end-to-end 검증, README 확정 | ✅ 완료 (§8 검증) |
| M5 | 시각화·서빙 (정적 사이트 Stack A) | ✅ 완료 ([Round 6](02-decision-rounds.md)) |
| M6 | 동적 웹앱 (Stack B), 의미 검색 | ⏳ 보류 |
